package fr.swapp.touch._.emulator 
{
	...
	public class TouchEmulator implements IDisposable
	{
		...
		/**
		 * Le multiplicateur pour détécter la direction (1 par défaut donc pas de diagonale)
		 */
		public static var directionDetectionMultiplier		:Number						= 1;
		
		/**
		 * Le délais de détéction entre TAP et DOUBLE_TAP, en ms. Par défaut 0 pour ne pas décaller le dispatch des taps.
		 */
		public static var doubleTapMaxDelay					:int						= 0;
		 ...
		 * 4 par défaut.
		 */
		public static var draggingThreshold					:uint						= 4;
		 ...
		 * @param	pMaxPoints : Le nombre de point maximum a prendre en compte sur le target. Entrer 0 pour ne prendre que le point principal. 2 par défaut.
		 * @return L'émulateur
		 */
		public static function emulate (pTarget:DisplayObject, pDelegate:ITouchDelegate = null, pMaxPoints:int = 2):TouchEmulator
		{
			// Créer / retourner
			return new TouchEmulator(pTarget, pDelegate, pMaxPoints);
		}
		 ...
		 * @param	pTarget : Le container sur lequel on veut émuler les GestureTouchEvent. Peut implémenter ITouchDelegate.
		 * @param	pDelegate : Le delegate de cette emulation (optionnel, si null le target sera utilisé s'il implémente les méthodes de délégation).
		 * @param	pMaxPoints : Le nombre de point maximum a prendre en compte sur le target. Entrer 0 pour ne prendre que le point principal, -1 pour ne pas limiter, 2 par défaut.
		 */
		public function TouchEmulator (pTarget:DisplayObject, pDelegate:ITouchDelegate = null, pMaxPoints:int = 2)
		{
			...
			_directionDetectionMultiplier = directionDetectionMultiplier;
		...
		protected function dispatchDragging ():void
		{
			...
			if (_delegate != null && _delegate is ITouchDragDelegate && _pointsIds[0] in _directions)
			{
				...
				if (
						...
						&&
						(_draggingNoMoveBuffer ++ <= draggingThreshold)
					)
				{
					// On ne dispatch pas
					return;
