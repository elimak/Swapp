package fr.swapp.graphic.text
{
	...
	import fr.swapp.graphic.controls.SCallout;
	 ...
	 * @author ZoulouX
	 */
	public class SHTMLText extends SText
	{
		/**
		 ...
		 * Les callouts
		 */
		protected var _callouts								:Vector.<SCallout>			= new Vector.<SCallout>;
		 ...
		 * Le constructeur
		 */
		public function SHTMLText (pAutoSize:Boolean = true, pMultiLine:Boolean = true, pTouchFriendlyLinks:Boolean = true, pValue:String = "")
		{
			// TODO : Revérifier le comportement de la classe.
			// TODO : Points sensibles : la méthode invalidate a été remplacée par invalidatePosition. La méthode needReplacer par replace.
			// TODO : Vérifier le système des liens
			// TODO : Vérifier le système de StyleSheets
		...
		{
			// Supprimer tous les callouts
			for each (var callout:SCallout in _callouts)
			{
		...
		protected function addCallout (pFirstIndex:int, pLastIndex:int, pID:int, pURL:String):void
		{
			...
			TimerUtils.wait(this, 1, true, function ():void
			{
				...
				{
					// Créer le callout
					var callout:SCallout = new SCallout(_linkTapColor, _linkTapAlpha, _linkTapBorderRadius, _linkTapDuration, _linkTapAnimated);
		 ...
		 * Clic sur un callout
		 */
		protected function calloutTappedHandler (pCallout:SCallout):void
		{
		 ...
		 * @param	pStyle : La CSS
		 */
		public function textStyle (pStyle:Object):SHTMLText
		{
			// Si on a un style
			if (pStyle != null)
			{
		 ...
		 */
		public function touchLinks (pEnabled:Boolean, pColor:int = 0x888888, pAlpha:Number = .4, pHorizontalPadding:int = 4, pVerticalPadding:int = 2, pDuration:Number = .5, pAnimated:Boolean = true):SHTMLText
		{
			// Enregistrer
