	...
	*/
	public class StringUtils {
		...
		*/
		...
		/**
		 * Formater un nombre pour qu'il soit JOLI
		 * @param	pNumber : Le nombre moche a rendre JOLI
		 * @param	pDecimals : Le nombre de décimales a afficher (2 par défaut)
		 * @param	pThousandSep : Le séparateur entre les milliers (un espace par défaut)
		 * @param	pDecimalSep : Le séparateur au niveau de la virgule (n'existe pas si 0 décimales)
		 * @return un JOLI nombre
		 */
		/*public static function formatNumber (pNumber:Number, pDecimals:uint = 2, pThousandSep:String = " ", pDecimalSep:String = ","):String
		{
			// TODO : Terminer la séparation des milliers
			var numParts:Array = pNumber.toFixed(pDecimals).split(".");
			
			var left:String = (numParts[0] as String);
			const total:uint = left.length;
			var leftParts:Array = [];
			
			//var out:String
			
			for (var i:int = total; i > 0; i -= 3)
			{
				trace(left.substr(i - 3, 3));
				
				//leftParts.unshift(left.substr(Math.max(0, i - 3), 3));
			}
			
			numParts[0] = leftParts.join(pThousandSep);
			
			//return out;
			
			return "TODO"//numParts.join(pDecimalSep);
		}*/
		
		
		/**
		 * Formater un nombre (Séparer les milliers, nombre de décimales...)
		 * @param	pNummber				Le nombre à traiter.
		 * @param	pDecimalDelimiter		Le séparateur pour les décimales (",", ".",...)
		 * @param	pThousandSep			Le séparateur des milliers.
		 * @param	pPrecision				Le nombre de décimales, si "0" on prend toutes les décimales.
		 * @param	pPrefix					Le préfixe.
		 * @param	pSuffix					Le sufixe.
		 * @return	Le nombre formaté.
		 */
		public static function formatNumber(pNumber:Number, pDecimalDelimiter:String = ".", pThousandSep:String = " ", pPrecision:int = 0, pPrefix:String = "", pSuffix:String = ""):String
		{
			var leftSide:String;
			var rightSide:String
			if (pPrecision > 0)
			{
				var decimalMultiplier:int = Math.pow(10, pPrecision);
				var str:String = Math.round(pNumber * decimalMultiplier).toString();				
				leftSide = str.substr(0, -pPrecision);
				rightSide = str.substr(-pPrecision);				
			}else {
				if (String(pNumber).indexOf(".") != -1)
				{
					leftSide = String(pNumber).split(".")[0];
					rightSide = String(pNumber).split(".")[1];
				}else
				{
					leftSide = String(pNumber);
					rightSide = "";
					pDecimalDelimiter = "";
				}
			...
			for (var i:int = 0;i < leftSide.length;i++)
			{
				if (i > 0 && (i % 3 == 0 ))
				{
					leftSideNew = pThousandSep + leftSideNew;
				}
					 
				leftSideNew = leftSide.substr(-i - 1, 1) + leftSideNew;
			} 
				   
			return pPrefix + leftSideNew + pDecimalDelimiter + rightSide + pSuffix;
		}

		
		protected static const LEFT_MUSTACHE		:String = '{{';
		protected static const RIGHT_MUSTACHE		:String = '}}';
		
		/**
		 * QuickMustache permet de faire un templating de pauvre.
		 * @param	pSentence : La phrase a templater (pas très français). Les séparateur sont des {{ }}. Par exemple : "Hello {{name}} !"
		 * @param	pValues : Les valeurs du template. Pour l'exemple précédent : {name: "world"}
		 * @return : la phrase templatée
		 */
		public static function quickMustache (pSentence:String, pValues:Object):String
		{
			for (var name:String in pValues)
			{
				pSentence = pSentence.replace(LEFT_MUSTACHE + name + RIGHT_MUSTACHE, pValues[name].toString());
			}
