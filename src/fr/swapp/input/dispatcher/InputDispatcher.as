package fr.swapp.input.dispatcher
{
	...
	import fr.swapp.input.errors.InputError;
	 ...
	 * @author ZoulouX
	 */
	public class InputDispatcher implements IDisposable
	{
		/**
		 * InputDispatcher instances
		 */
		protected static var __instances				:Dictionary					= new Dictionary(false);
		
		
		/**
		 * Get a InputDispatcher instance for a specific stage. Will create an instance for a stage if not created yet.
		 * @param	pStage : Associated stage. Must be non null.
		 * @param	pEnableTouch : Enable touch managment
		 ...
		 */
		public static function getInstance (pStage:Stage, pEnableTouch:Boolean = true, pEnableMouse:Boolean = true, pTapThreshold:int = 0):InputDispatcher
		{
			// Vérifier que le stage ne soit pas null
			if (pStage == null)
			{
				throw new InputError("InputDispatcher.getInstance", "Stage can't be null.");
				return null;
			}
			...
			{
				// On l'a créé
				new InputDispatcher(new MultitonKey(), pStage, pEnableTouch, pEnableMouse, pTapThreshold);
			}
		...
		/**
		 * Direction multiplier to detect direction vertical / horizontal or both.
		 * If 1, direction can't be unknown.
		 */
		...
		/**
		 * Protected constructor. Please use InputDispatcher.getInstance(pStage).
		 */
		public function InputDispatcher (pMultitonKey:MultitonKey, pStage:Stage, pEnableTouch:Boolean, pEnableMouse:Boolean, pTapThreshold:int)
		{
			// TODO : gestion des swipes via IInputSwipeDelegate
			// TODO : gestion des Double taps
			...
			if (pMultitonKey == null)
			{
				...
				throw new InputError("InputDispatcher.construct", "Direct instancation not allowed, please use InputDispatcher.getInstance instead.");
			}
			else
			{
				// Appeler le sous-constructeur
				construct(pStage, pEnableTouch, pEnableMouse, pTapThreshold);
			}
		}
		 ...
		 * Second constructor
		 */
		protected function construct (pStage:Stage, pEnableTouch:Boolean, pEnableMouse:Boolean, pTapThreshold:int):void
		{
			// Enregistrer l'instace de ce dispatcher par rapport au stage
			...
			// Si on doit gérer les touch events
			if (pEnableTouch)
			{
			...
			if (target != null)
			{
				...
				while (target != _stage)
				{
					...
					if (target is IInputDelegate)
					{
						...
						if (target is IInputDragDelegate)
						{
							registerDragDelegate(target as IInputDragDelegate, event);
						}
						if (target is IInputSingleDelegate)
						{
							registerSingleDelegate(target as IInputSingleDelegate, event);
						}
						if (target is IInputTransformDelegate)
						{
							registerTransformDelegate(target as IInputTransformDelegate, event);
						}
					}
		...
		protected function touchMoveHandler (event:TouchEvent):void 
		{
			...
			if (touchId in _draggables && touchId in _directions)
			{
				...
				var allowed:Boolean;
				for each (var dragDelegate:IInputDragDelegate in _draggables[touchId])
				{
					...
					allowed = dragDelegate.inputDragging(
						InputTypes.TOUCH_INPUT,
						_targets[touchId],
						_directions[touchId],
			...
			if (touchId in _transformables)
			{
				...
				for each (var transformDelegate:IInputTransformDelegate in _transformables[touchId])
				{
		...
		protected function touchEndHandler (event:TouchEvent):void 
		{
			...
			{
				// Les parcourir
				for each (var touchTapDelegate:IInputSingleDelegate in _touchables[touchId])
				{
					...
					else if (_pressedTouchables[touchTapDelegate] <= 1)
					{
						...
						touchTapDelegate.inputReleaseHandler(InputTypes.TOUCH_INPUT, _targets[touchId], event.isPrimaryTouchPoint);
						...
						{
							// Dispatcher le tap
							touchTapDelegate.inputSingleHandler(InputTypes.TOUCH_INPUT, _targets[touchId], event.isPrimaryTouchPoint);
						}
			...
			{
				// Les parcourir
				for each (var touchTransformDelegate:IInputTransformDelegate in _transformables[touchId])
				{
					...
					if (touchTransformDelegate in _pressedTransformables)
					{
						...
						if (_pressedTransformables[touchTransformDelegate].length == 1)
						{
							...
							touchTransformDelegate.inputTransformStopHandler(InputTypes.TOUCH_INPUT, _targets[touchId]);
						}
		 ...
		 */
		protected function registerDragDelegate (pDelegate:IInputDragDelegate, pEvent:TouchEvent):void
		{
			// Cibler l'id du touch
			...
			pDelegate.inputDragLock(InputTypes.TOUCH_INPUT, _targets[touchId]);
		}
		 ...
		 * @param	pEvent : The touch start event firing
		 */
		protected function registerSingleDelegate (pDelegate:IInputSingleDelegate, pEvent:TouchEvent):void
		{
			// Cibler l'id du touch
			var touchId:int = pEvent.touchPointID;
			...
			if (!(pDelegate in _pressedTouchables))
			{
				...
				// Dispatcher
				pDelegate.inputPressHandler(InputTypes.TOUCH_INPUT, _targets[touchId], pEvent.isPrimaryTouchPoint);
			}
			else
			{
				// Ajouter ce point
		 ...
		 * @param	pEvent : The touch start event firing
		 */
		protected function registerTransformDelegate (pDelegate:IInputTransformDelegate, pEvent:TouchEvent):void
		{
			// Cibler l'id du touch
			var touchId:int = pEvent.touchPointID;
			...
			if (_pressedTransformables[pDelegate].length == 2)
			{
				...
				var newTarget:DisplayObject = pDelegate.inputTransformStartHandler(InputTypes.TOUCH_INPUT, _targets[touchId]);
		...
		{
			// Le delegate ciblé
			var transformDelegate		:IInputTransformDelegate;
			...
			for (var i:* in _pressedTransformables)
			{
				...
				transformDelegate = (i as IInputTransformDelegate);
				...
				if (!(transformDelegate in _currentCenter))
				{
					...
					_currentTransformType = transformDelegate.inputTransformMatrixType(InputTypes.TOUCH_INPUT, target);
				...
				else
				{
					...
					transformDelegate.inputTransformHandler(InputTypes.TOUCH_INPUT, target, distanceOffset, angleOffset, centerHorizontalOffset, centerVerticalOffset, pointIds.length);
					...
					// Appliquer le scale
					if (_currentTransformType & InputMatrixOptions.SCALE_OPTION)
					{
						matrix.scale(distanceOffset, distanceOffset);
					}
					
					// Appliquer la rotation
					if (_currentTransformType & InputMatrixOptions.ROTATION_OPTION)
					{
					...
					if (_currentTransformType & InputMatrixOptions.DRAG_OPTION)
					{
						matrix.translate(centerHorizontalOffset, centerVerticalOffset);
					}
					
					// Dispatcher la transformation
					transformDelegate.inputMatrixTransformHandler(InputTypes.TOUCH_INPUT, target, matrix, pointIds.length);
				}
			}
		}
		...
		protected function computeDirection (event:TouchEvent):void
		{
			...
			if (!(touchId in _directions))
			{
				...
				else if (Math.abs(_touchDeltas[touchId].x) > Math.abs(_touchDeltas[touchId].y) * _directionDetectionMultiplier)
				{
					...
					_directions[touchId] = InputDirections.HORIZONTAL_DIRECTION;
				}
				...
				else if (Math.abs(_touchDeltas[touchId].y) > Math.abs(_touchDeltas[touchId].x) * _directionDetectionMultiplier)
				{
					...
					_directions[touchId] = InputDirections.VERTICAL_DIRECTION;
				}
				...
				else
				{
					...
					_directions[touchId] = InputDirections.UNKNOW_DIRECTION;
				}
			}
		}
		...
		public function dispose ():void
		{
			...
			throw new Error("NOT IMPLEMENTED YET -> InputDispatcher");
